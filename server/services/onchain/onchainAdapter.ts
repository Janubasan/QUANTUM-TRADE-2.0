...
import { OnchainService, getOnchainService, NATIVE_ALIASES } from './onchainService.js';
...
export class OnchainAdapter implements BrokerAdapter {
  ...
  private readonly _service?: OnchainService;

  public get service(): OnchainService {
    if (this.tokenMap && this._service) {
      return this._service;
    }
    return getOnchainService();
  }
  ...
  /** Endereço da carteira configurada em memória (ou vazio se não configurado). */
  public get address(): string {
    return this.service.walletState().address;
  }

  /** Compatibilidade com código legado: retorna a definição da rede ativa. */
  public getChain(): ChainDefinition {
    return this.chain;
  }

  /** Compatibilidade com código legado: altera a rede ativa no OnchainService. */
  public async setChain(chainKey: any): Promise<ChainDefinition> {
    return this.service.setChain(chainKey);
  }

  /** Compatibilidade com código legado: saldo nativo real formatado. */
  public async getNativeBalance(): Promise<string> {
    const native = await this.service.getNativeBalance();
    return native.amount;
  }

  /** Compatibilidade com código legado: cotação real. */
  public async getSwapQuote(tokenIn: string, tokenOut: string, amountIn: string, slippageBps = 50) {
    const quote = await this.service.getQuote(tokenIn, tokenOut, amountIn, slippageBps);
    return quote.path.map((_, idx) => (idx === 0 ? amountIn : quote.amountOutRaw));
  }

  /** Compatibilidade com código legado: swap real. */
  public async swapTokens(tokenIn: string, tokenOut: string, amountIn: string, slippageBps = 50, confirm = false) {
    return this.service.swap({
      tokenIn,
      tokenOut,
      amountIn,
      slippageBps,
